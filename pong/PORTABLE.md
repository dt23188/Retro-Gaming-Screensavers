...
```sh
bash pong/install-linux.sh
# Require offline installation; fail clearly if no compatible payload exists:
bash pong/install-linux.sh --offline
```

The command is `~/.local/bin/retro-pong-screensaver`; add `--preview` for a window.
...
```sh
swayidle -w timeout 600 "$HOME/.local/bin/retro-pong-screensaver"
```
...
Double-click `pong/install-macos.command`. With a matching native runtime and
...
for Pong. Select `RetroPong` in System Settings > Screen Saver.

The native bundle is in `~/Library/Screen Savers/RetroPong.saver`; keep its
runtime under `~/Library/Application Support/RetroGamingScreensavers/pong`.
...
copy the matching `runtimes/PLATFORM-ARCH` folder into `pong/runtimes/` first, or
