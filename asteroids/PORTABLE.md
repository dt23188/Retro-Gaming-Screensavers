...
```sh
bash asteroids/install-linux.sh
# Require offline installation; fail clearly if no compatible payload exists:
bash asteroids/install-linux.sh --offline
```

The command is `~/.local/bin/retro-asteroids-screensaver`; add `--preview` for a window.
...
```sh
swayidle -w timeout 600 "$HOME/.local/bin/retro-asteroids-screensaver"
```
...
Double-click `asteroids/install-windows.cmd`. If `runtimes/windows-x86_64` exists,
...
Installation lives under `%LOCALAPPDATA%\RetroGamingScreensavers\asteroids`.
...
Double-click `asteroids/install-macos.command`. With a matching native runtime and
...
for Pong. Select `RetroAsteroids` in System Settings > Screen Saver.

The native bundle is in `~/Library/Screen Savers/RetroAsteroids.saver`; keep its
runtime under `~/Library/Application Support/RetroGamingScreensavers/asteroids`.
...
copy the matching `runtimes/PLATFORM-ARCH` folder into `asteroids/runtimes/` first, or
